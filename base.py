...
from .acggov import acggov_fetch_process, acggov_get_ranking, acggov_get_ranking_setu, acggov_get_setu, acggov_init, \
    acggov_search_setu, acggov_get_setu_native, save_image, get_details, download_acggov_image
...
def get_spec_image(ids: str):
    ...
    if image is None or image['data'] is None:
        image = acggov_get_setu_native(uid=int(ids))

        if image is None or image['data'] is None:
            image_data = get_details(int(ids))

            image = {
                'id': image_data['data']['illust']['id'],
                'url': image_data['data']['illust']['meta_single_page']['original_image_url'],
                'title': image_data['data']['illust']['title'],
                'author': image_data['data']['illust']['user']['name'],
                'tags': image_data['data']['illust']['tags'],
                'data': None,
                'native': True,
            }
            url = image['url'].replace("https://i.pximg.net", "https://i.acgmx.com")
...
async def get_ranking(group_id, page: int = 1):
